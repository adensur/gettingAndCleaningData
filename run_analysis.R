...
#getting the features names,which will serve as our column names (this is 
...
#getting activity labels and adding them as another, 562-th column of train and 
#test data:
...
#getting ids of subjects, who performed each activity and adding them as 563th 
#column of the data:
...
#last step before merging train and test data - I want to keep the information
#anout whether the subject performed in the test data set or in the train data
#set, so I am creating an extra column called "dataset"
train$dataset="train"
test$dataset="test"

#merging the data (this corresponds to the 1st step in the assignement):
data=rbind(train,test)
rm(train)
rm(test)

#getting activity names for each activity label and merging them with our 
...
#to find out which columns correspond to "mean" or "standard deviation" values,
...
#the tidy data for step4 is ready! Let us save it and move on to the 5th step:
write.csv(data,"tidy1.csv",quote=FALSE,row.names=FALSE)


#step 5: calculating mean of each variable for each activity and each subject:
...
write.csV(data2,"tidy2.csv",quote=FALSE,row.names=FALSE)
